// OctoCRM Real-time Sync & Conflict Visualizer Patch
(function(){
    ...
    if (cloud && cloud.records && local && local.records) {
      for (const id in cloud.records) {
        ...
        if (l && c && c._modifiedAt && l._modifiedAt && c._modifiedAt !== l._modifiedAt) {
          LAST_SYNC_CONFLICTS.push({id, local:l, cloud:c});
        }
      }
    }
  ...
  const refresh = window.refreshPage || function(){ try{ window.location.reload(); }catch(e){} };
  ...
  window.fbDB.ref(FB_DB_NODE).on('value', snap => {
    ...
    if (_fbInitialLoad) {
      ...
      saveLocal(); refresh(); updSync('ok'); return;
    }
  ...
  window.renderConflictBadge = function(){
    const el = document.getElementById('syncConflicts'); if(!el) return;
    if (!window.LAST_SYNC_CONFLICTS.length){ el.style.display='none'; return; }
    el.style.display='inline-block'; el.textContent = '⚠ ' + window.LAST_SYNC_CONFLICTS.length;
  };
  ...
  window.showSyncConflicts = function(){
    ...
    if (window.showModal) window.showModal(html); else alert(html.replace(/<[^>]+>/g,''));
  };

  window.resolveConflict = function(id, choice){
    const c = window.LAST_SYNC_CONFLICTS.find(x=>x.id===id); if(!c) return;
    window.DB.records[id] = choice==='local' ? c.local : c.cloud;
    saveLocal(); if (window.pushToCloud) window.pushToCloud(); refresh();
  };

  console.log('OctoCRM real-time sync patch applied');
})();
